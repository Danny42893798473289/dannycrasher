package me.dannycrasher.client.config;
...
import me.dannycrasher.client.command.impl.HideNickCommand;
...
public class ClientConfigManager {
	...
	private final Path configPath = FabricLoader.getInstance().getConfigDir().resolve("dannycrasher").resolve("server-configs.json");
	...
	public void loadCurrentServer() {
		...
		DannyCrasher.getCommandManager().applyPrefix(serverConfig.prefix);
		DannyCrasher.getClickGuiManager().setConfigs(serverConfig.moduleConfigs);
        HideNickCommand.setEnabled(serverConfig.hideNick);
    }

	public void saveCurrentServer() {
		ServerConfig serverConfig = configFile.servers.computeIfAbsent(activeServerKey, ignored -> new ServerConfig());
		serverConfig.prefix = DannyCrasher.getCommandManager().getPrefix();
		serverConfig.moduleConfigs = DannyCrasher.getClickGuiManager().getConfigs();
