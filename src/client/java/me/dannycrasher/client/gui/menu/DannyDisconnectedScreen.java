...
import me.dannycrasher.client.mixin.DisconnectedScreenAccessor;
...
public class DannyDisconnectedScreen extends Screen {
	...
	private final MenuButton mainMenuButton = new MenuButton("Main Menu", "Back to Danny Crasher home", () ->
			minecraft.setScreen(new MainMenuScreen()));

	public DannyDisconnectedScreen(Component title, Component reason) {
		super(title);
		this.reason = reason == null ? Component.empty() : reason;
	}

	public static DannyDisconnectedScreen fromVanilla(DisconnectedScreen screen) {
		Component reason = ((DisconnectedScreenAccessor) screen).dannycrasher$getDetails().reason();
		return new DannyDisconnectedScreen(screen.getTitle(), reason);
	}
