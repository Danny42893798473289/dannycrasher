...
import me.dannycrasher.client.alt.AltProfile;
import me.dannycrasher.client.utils.PlayerHeadUtil;
...
public class AltManagerScreen extends Screen {
	...
	public AltManagerScreen(Screen parent) {
		super(Component.literal("Danny Crasher Alt Manager"));
		this.parent = parent;
	}
	...
	protected void init() {
		...
		List<AltProfile> profiles = DannyCrasher.getAltManager().getProfiles();
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		...
		int maxScroll = Math.max(0, DannyCrasher.getAltManager().getProfiles().size() - getVisibleRows());
	...
	private void renderSidePanel(GuiGraphics graphics, int mouseX, int mouseY) {
		...
		graphics.drawString(font, trimToWidth(DannyCrasher.getAltManager().getStatus(), sideWidth - 36), x + 18, y + 36, MenuTheme.SOFT_TEXT, true);
		...
		renderSmallButton(graphics, getActionX(3), getActionY(3), getActionWidth(), 28, "Microsoft", mouseX, mouseY, !DannyCrasher.getAltManager().isMicrosoftLoginInProgress());
	...
	private void addOffline() {
		DannyCrasher.getAltManager().addOfflineProfile(nameInput.getValue());
		nameInput.setValue("");
		selectedIndex = DannyCrasher.getAltManager().getProfiles().size() - 1;
	}

	private void loginSelected() {
		if (hasSelectedProfile()) {
			DannyCrasher.getAltManager().login(DannyCrasher.getAltManager().getProfiles().get(selectedIndex));
		}
	}
	...
	private void removeSelected() {
		...
		AltProfile profile = DannyCrasher.getAltManager().getProfiles().get(selectedIndex);
		DannyCrasher.getAltManager().remove(profile);
		PlayerHeadUtil.invalidate(profile);
		selectedIndex = Math.min(selectedIndex, DannyCrasher.getAltManager().getProfiles().size() - 1);
	}

	private void startMicrosoftLogin() {
		DannyCrasher.getAltManager().startMicrosoftLogin(() -> selectedIndex = DannyCrasher.getAltManager().getProfiles().size() - 1);
	}
	...
	private int getClickedProfileIndex(double mouseX, double mouseY) {
		for (int index = 0; index < getVisibleRows(); index++) {
			...
			if (profileIndex < DannyCrasher.getAltManager().getProfiles().size() && isInside(mouseX, mouseY, getListX() + 14, rowY, getListWidth() - 28, ROW_HEIGHT - 8)) {
				return profileIndex;
			}
		}
	...
	private boolean hasSelectedProfile() {
		return selectedIndex >= 0 && selectedIndex < DannyCrasher.getAltManager().getProfiles().size();
	}
