...
import me.dannycrasher.client.command.CommandParseException;
...
public class ModuleManager {
	...
	public void executeModule(String name, List<String> arguments) {
		...
		if (module == null) {
			...
			DannyCrasher.getNotificationManager().show(NotificationType.ERROR, "Module", "Unknown module: " + name);
			return;
		}

		if (arguments.size() != module.getArguments().size()) {
			sendClientMessage("Usage: " + module.getUsage(DannyCrasher.getCommandManager().getPrefix()));
			DannyCrasher.getNotificationManager().show(NotificationType.WARNING, module.getName(), "Invalid arguments");
			return;
		}

		DannyCrasher.getNotificationManager().show(NotificationType.INFO, module.getName(), "Executing module...");

		try {
			module.executeModule(new ModuleContext(this, module, parseArguments(module, arguments), List.copyOf(arguments)));
			DannyCrasher.getNotificationManager().show(NotificationType.SUCCESS, module.getName(), "Module executed successfully");
		} catch (CommandParseException exception) {
			sendClientMessage(exception.getMessage());
			sendClientMessage("Usage: " + module.getUsage(DannyCrasher.getCommandManager().getPrefix()));
			DannyCrasher.getNotificationManager().show(NotificationType.WARNING, module.getName(), exception.getMessage());
